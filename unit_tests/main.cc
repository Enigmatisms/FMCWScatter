...
int main(int argc, char** argv) {
    ...
    std::vector<double> spect(4096);
    size_t result_size = 0;
    double range_gt = 24., range_pred = 0., vel_gt = -4.0, vel_pred = 0.;
    auto start_t = std::chrono::system_clock::now();
    simulateOnce(params, spect.data(), range_pred, vel_pred, result_size, range_gt, vel_gt, 2 * M_PI * 50e6);
