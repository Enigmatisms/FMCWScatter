...
extern "C" {
    void setup_path_tracer(int ray_num) {
        ...
        printf("Ray num: %d, memory allocated\n", ray_num);
    }
    ...
    void get_intersections_update(Vec2* const intersections, int mesh_num, int aabb_num) {
        ...
        path_tracer.scattering_event();     // mean free path sampling
    ...
    void static_scene_update(
        const Vec2* const meshes, const ObjInfo* const host_objs, const World& h_world, 
        const short* const host_inds, const char* const host_nexts, size_t line_seg_num, size_t obj_num
    ) {
        printf("Seg num: %lu\n", line_seg_num);
        ...
        // TODO: Can we get address by '&'? Will we get host address via this?
        CUDA_CHECK_RETURN(cudaMemcpyToSymbol(world, &h_world, sizeof(World), 0, cudaMemcpyHostToDevice));
