...
PathTracer::PathTracer(size_t ray_num):
    ...
    ray_d(nullptr, get_deletor<float>()), ranges(nullptr, get_deletor<float>()),
    mesh_inds(nullptr, get_deletor<short>()), ray_num(ray_num)
{
    ...
    CUDA_CHECK_RETURN(cudaMalloc((void **) &cu_ray_d, ray_num * sizeof(float)));
    ...
    CUDA_CHECK_RETURN(cudaMallocHost((void **) &ray_d_ptr, ray_num * sizeof(float)));
